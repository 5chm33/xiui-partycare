...
partyList.HandleZonePacket = function(e)
    -- Clear cast data and observed upkeep timing on zone.
    data.partyCasts = {};
    partyCare.Reset();
    partyList.partyCasts = data.partyCasts;
end
...
partyList.HandleActionPacket = function(actionPacket)
    ...
    if (actionPacket.Type == 8) then
        if (actionPacket.Targets and #actionPacket.Targets > 0 and
            ...
            if (existingCast ~= nil and existingCast.spellId == spellId) then
                -- XIUI treats a repeated start for the same spell as an interrupt.
                partyCare.ObserveSpellResult(actionPacket.UserId, true, os.clock());
                data.partyCasts[actionPacket.UserId] = nil;
                return;
            end

            if (existingCast ~= nil and existingCast.spellId ~= spellId) then
                -- A different new cast replaces the previous in-flight cast.
                partyCare.ObserveSpellResult(actionPacket.UserId, true, os.clock());
                data.partyCasts[actionPacket.UserId] = nil;
            end
            ...
            if (spell ~= nil and spell.Name[1] ~= nil) then
                ...
                -- Keep Refresh/Haste observation separate from the visual cast
                -- bar, which XIUI may clear as soon as it reaches 100 percent.
                local localPlayerId = party and party:GetMemberServerId(0) or nil;
                if actionPacket.UserId == localPlayerId then
                    partyCare.ObserveStartedSpell(actionPacket.UserId, spellId, actionPacket.Targets[1].Id, os.clock());
                end
    ...
    elseif (actionPacket.Type == 4 or actionPacket.Type == 11) then
        local party = GetPartySafe();
        local localPlayerId = party and party:GetMemberServerId(0) or nil;
        local completedCast = data.partyCasts[actionPacket.UserId];
        local finishMessage = nil;
        if (actionPacket.Targets and #actionPacket.Targets > 0 and
            actionPacket.Targets[1].Actions and #actionPacket.Targets[1].Actions > 0) then
            finishMessage = actionPacket.Targets[1].Actions[1].Message;
        end
        local interrupted = finishMessage == 16 or finishMessage == 84 or finishMessage == 106;

        -- Resolve the separately retained pending upkeep cast even if XIUI's
        -- visual cast bar was already cleared at 100 percent.
        if actionPacket.UserId == localPlayerId then
            partyCare.ObserveSpellResult(actionPacket.UserId, interrupted, os.clock());
        end
        data.partyCasts[actionPacket.UserId] = nil;
    end
