...
local partyCare = require('modules.partylist.partycare');
...
function display.DrawMember(memIdx, settings, isLastVisibleMember)
    ...
    -- A high-visibility, full-width remedy strip is rendered directly inside
    -- the Party List window, beneath the actual XIUI party card. The reserved
    -- row keeps the action visible and hit-tested by XIUI's normal input path.
    ...
    local remedyButtonHeight = showRemedyButton and math.max(18, nameRefHeight + 6) or 0;
    if showRemedyButton then
        local entryStartY = hpStartY - nameRefHeight - settings.nameTextOffsetY;
        local savedCursorX, savedCursorY = imgui.GetCursorScreenPos();
        imgui.SetCursorScreenPos({hpStartX, entryStartY + entryHeight + 3});
        ...
        imgui.SetCursorScreenPos({savedCursorX, savedCursorY});
    end
    ...
    if (memInfo.inzone and not isPreviewMode and not showConfig[1] and needsEntryHitRegion) then
        ...
        local entryClicked = imgui.InvisibleButton('PartyMemberEntry' .. memIdx, {allBarsLengths, entryHeight});
        local partyCareConsumedClick = false;
        if hoverCareEnabled and imgui.IsItemHovered() then
            -- Button ids follow ImGui's standard ordering: left, right, middle,
            -- Mouse 4, and Mouse 5.  A configured manual care binding consumes
            -- that click so it cannot also retarget through XIUI's click target.
            for button = 0, 4 do
                if imgui.IsItemClicked(button) and partyCare.IsMouseButtonBound(button) then
                    partyCare.HandleMouseButton(memIdx, button);
                    partyCareConsumedClick = true;
                end
            end
            local io = imgui.GetIO();
            local wheelDelta = io and tonumber(io.MouseWheel) or 0;
            if wheelDelta ~= 0 then
                partyCare.HandleHoverWheel(careState, memIdx, wheelDelta);
            end
        end
        if entryClicked and gConfig.enablePartyListClickTarget and not partyCareConsumedClick then
