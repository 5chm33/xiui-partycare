...
The integration maps only direct, well-defined status icons to remedies. When a verified status has an eligible local remedy, it renders a **full-width red `REMEDY: <spell>` strip beneath that party card**. The control is now rendered in its own top-layer XIUI overlay after the party window, rather than inside the card’s clipped draw region, so it cannot be covered by native card, status-icon, or background layers. Its own reserved space prevents overlap with the next party member. Generic `stat_down` / Evasion Down-style icon groups remain intentionally excluded because they can be ambiguous or transient around Level Sync and cannot safely be translated to one universal Erase recommendation.
